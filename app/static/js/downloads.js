(function(){
  ...
  async function analyze() {
    ...
    try {
      const resp = await fetch('/downloads/analyze', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ url }), credentials: 'same-origin' });
      if (!resp.ok) { const t = await resp.text(); alert('Analyze failed: '+t); return; }
      const data = await resp.json();
      el('dl-title').textContent = data.title || '';
      const vf = el('video-format'); const af = el('audio-format');
      vf.innerHTML = ''; af.innerHTML = '';
